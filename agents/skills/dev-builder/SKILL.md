...
[第一性原则]
    ...
    测试纪律：所有有运行时行为的代码（除工具链自动生成的中间代码外），生产代码前必有失败的测试。AI 生成代码等同手写，不享受例外。
    代码精简：三行直白代码好过一个过度抽象。
...
[开发规则清单]
    ...
    [测试纪律]
        铁律：所有有运行时行为的代码 TDD 必走 RED → Verify RED → GREEN → Verify GREEN → REFACTOR → Verify GREEN stays。无失败测试不写生产代码。
        RED：先写一个最小失败测试，命名回答"什么生产变更会让它失败"。跑测试确认 FAIL 且失败原因是"功能缺失"而非笔误。AI 生成的代码不豁免 RED。
        GREEN：写最小代码让测试 PASS。禁止超实现——不写测试没要求的功能、不"优化"无关代码、不重构。跑全套测试零失败 + 输出干净（无 error/warning）。
        REFACTOR：保持绿色前提下清重复、改命名、抽公共方法。不新增行为。
        预期值必须独立推导：手算、字面量、表驱动。不用被测代码生成期望（mirror assertion 必揪）。
        断言测真实行为，不测 mock 存在（expect(mock).toBe... 是无效断言）。
        mock 只隔离慢/外部边界，镜像真实数据结构全字段，不是只镜像测试读到的字段。
        mock setup 超过测试逻辑一半就改集成测试。
        测试只用的清理方法放 test utilities，不进生产类。
        跑完测试跑 mutation check：脑里改一处生产代码（错常量、错分支、漏副作用、空返回、漏校验），至少一个测试要失败。
        边界与错误用例：空、零、nil、未授权、畸形输入。
        测试名有"and"就拆。一个 break 一段测试。
        例外：工具链自动生成的中间代码（protobuf、Prisma client、codegen 产物）不写测试；纯 schema/配置可写 contract test；throwaway 原型不强制。
...
[Phase 执行流程]
    ...
    每个 Task 强制走 RED-GREEN-REFACTOR + review→fix，不跳步：
        RED：读 DEV-PLAN 交付清单、Spec 功能描述、Design-Brief 视觉方向的原文（不凭记忆）。为本 Task 每个行为点写最小失败测试，测试名回答"什么 break 让它失败"。跑测试确认 FAIL 且失败原因是"功能缺失"非笔误或 typo。AI 生成代码不豁免 RED。看不到失败不准进 GREEN。
        GREEN：写最小代码让测试 PASS。禁止超实现（不写测试没要求的功能、不重构无关代码、不"优化"测试覆盖外的部分）。跑全套测试确认零失败 + 输出干净无 error/warning。
        REFACTOR：保持绿色前提下清重复、改命名、抽公共方法。不新增行为。跑测试确认仍全绿。
        自检：代码实际值对照设计数值，行为对照 Spec。
        派发 code-reviewer 两阶段审查；Stage 2 检 TDD 合规（测试是否先写、断言是否独立、mock 是否替断言、mutation check 是否过）。
        Stage 1 失败回 RED 补实现，重新派 code-reviewer。
        Stage 2 失败：质量和重构问题自己按修改纪律修，确属缺陷或安全漏洞才调 bug-fixer（bug-fixer 必先写复现测试），重新派 code-reviewer。
        两阶段都过 → echo clean > .codex/.needs-review → commit（feat:/fix:/test:/refactor: 前缀区分，区分 TDD 步骤里的 test: 与 fix:） → 下一个 Task。
        用户强调某环节是追加要求不替换基础流程，review 闭环与 TDD 循环照常走。
    用户强调某环节是追加要求，不替换基础流程，review 闭环照常走。

[Phase 完成度判断]
    所有 Task 完成后过四步走，全通过才算 Phase 完成：
        一、Code Review：对照 DEV-PLAN 交付清单逐项确认，检查有无超范围改动
        二、测试完整性 + TDD 合规：计划的功能都实现无半成品，且测试满足以下纪律——
        每个行为点都有测试，测试 commit 时间早于或同于生产代码（git blame 抽查），不算 TDD 的标"测试后挂"。
        每条「绿」能证明行为真的对：独立推导预期值（手算、字面量、表驱动），不用被测代码生成。
        用假前提或不可达输入把缺陷盖成预期等于没测，揪出。
        断言测真实行为，不测 mock 存在（expect(mock).toBe... 是无效断言）。
        功能声明的错误态、空态、边界要有用例真的走到，不只在实现里留分支。
        mutation check 过：脑里改一处生产代码（错常量、错分支、漏副作用、空返回、漏校验），至少一个测试要失败；全不挂 = 测试是装饰。
